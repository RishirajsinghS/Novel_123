CLASS zcl_packn_file_upload DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_PACKN_FILE_UPLOAD IMPLEMENTATION.
  ...
  METHOD get_html.
    ...
    |<!DOCTYPE HTML> \n| &&
     ...
     |    <title>ZPACKN File Uploader</title> \n| &&
     ...
     |                shell.setTitle("ZPACKN File Uploader") \n| &&
...
*     |                    \}) \n| &&
     ...
     |                    rbDelete = new sap.m.RadioButton("rbDelete") \n| &&
     |                    rbModify = new sap.m.RadioButton("rbModify") \n| &&
     ...
     |                    groupDataOptions.addButton(rbDelete) \n| &&
     |                    groupDataOptions.addButton(rbModify) \n| &&
     |                    rbAppend.setGroupName("grpDataOptions") \n| &&
     |                    rbReplace.setGroupName("grpDataOptions") \n| &&
     |                    rbDelete.setGroupName("grpDataOptions") \n| &&
     |                    rbModify.setGroupName("grpDataOptions") \n| &&
...
* Load the data to the table via dynamic internal table
        ...
        TRY.
            CREATE DATA dynamic_table TYPE TABLE OF zpackn.
        ...
        IF dataoption = '0' OR dataoption = '1'.   "for append and insert
          IF dataoption = `1`.  "if replace, delete the data from the table first
            DELETE FROM zpackn.
          ENDIF.
          TRY.
              INSERT zpackn FROM TABLE @<table_structure>.
              IF sy-subrc = 0.
            ...
            CATCH cx_sy_open_sql_db INTO DATA(db_exception).
              ...
              RETURN.
          ENDTRY.
        ELSEIF dataoption = '2'.           "for deleting data
          TRY.
              DATA: where TYPE string.
              DELETE zpackn FROM TABLE @<table_structure>.
              IF sy-subrc = 0.
                response->set_status( i_code = if_web_http_status=>ok
                                      i_reason = 'Table deleted successfully' ).
                response->set_text( 'Table deleted successfully' ).
              ENDIF.
            CATCH cx_sy_open_sql_db INTO db_exception.
              response->set_status( i_code = if_web_http_status=>bad_request
                                   i_reason = db_exception->get_text(  ) ).
              response->set_text( db_exception->get_text(  )  ).
              RETURN.
        ...
        ELSEIF dataoption = '3'.
          TRY.
              MODIFY zpackn FROM TABLE @<table_structure>.
              IF sy-subrc = 0.
                response->set_status( i_code = if_web_http_status=>ok
                                      i_reason = 'Table modified successfully' ).
                response->set_text( 'Table modified successfully' ).
              ENDIF.
            CATCH cx_sy_open_sql_db INTO db_exception.
              response->set_status( i_code = if_web_http_status=>bad_request
                                   i_reason = db_exception->get_text(  ) ).
